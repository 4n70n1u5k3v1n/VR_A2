...
public class FinalDoorTrigger : MonoBehaviour
{
    ...
    public float openedHeight = 3.0f;   // opened door height
    ...
    void Start()
    {
        ...
        openedPos.y += openedHeight; // Use += to move forward, -= to move backward

        // Get AudioSource from mummy
        if (mummy != null)
        {
            groanAudio = mummy.GetComponent<AudioSource>();
        }
    }
    ...
    void OnTriggerExit(Collider other)
    {
        ...
        // Stop groaning
        if (groanAudio != null && groanAudio.isPlaying)
        {
            groanAudio.Stop();
        }
    }
